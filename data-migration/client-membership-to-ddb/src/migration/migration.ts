...
function findClientIdFromGroups(groups: string[]): string[] {
...
async function migrateSingleUser(
  ...
  const clientIds = findClientIdFromGroups(groups);
...
export async function runMigration() {
  ...
  await backupDataToS3(usersWithGroups, env);

  // Migrate data to DynamoDB
  let countMigrated = 0;
  let countSkipped = 0;
  const sortedUsers = usersWithGroups.toSorted((a, b) =>
  ...
  for (const userWithGroups of sortedUsers) {
    ...
    logger.info(
      `Migrated ${countMigrated}, Skipped ${countSkipped}, Total ${countMigrated + countSkipped}/${usersWithGroups.length} (${(100 * (countMigrated + countSkipped)) / usersWithGroups.length}%) Processed user ${user.Username}`
    );
  }
  ...
  await deleteEmptyClientGroups(userPoolId, dryRun);
