...
resource "aws_iam_policy" "identity_pool_unauthenticated_policy" {
  name        = "${local.csi}-unauthenticated-policy"
  description = "Policy for unauthenticated users"
  policy      = data.aws_iam_policy_document.identity_pool_role_policy.json
}

resource "aws_iam_role_policy_attachment" "identity_pool_unauthenticated_role_attachment" {
  role       = aws_iam_role.identity_pool_unauthenticated_role.name
  policy_arn = aws_iam_policy.identity_pool_unauthenticated_policy.arn
}
