...
test('fails with invalid id_assurance_level', async () => {
  ...
  await expect(() =>
    handler(event, mockDeep<Context>(), mockDeep<Callback>())
  ).rejects.toThrow('Invalid id_assurance_level. Expected 3');
});
...
test('fails with invalid authentication_assurance_level', async () => {
  ...
  await expect(() =>
    handler(event, mockDeep<Context>(), mockDeep<Callback>())
  ).rejects.toThrow('Invalid authentication_assurance_level. Expected 3');
});
...
test('fails with invalid auth_time', async () => {
  ...
  await expect(() =>
    handler(event, mockDeep<Context>(), mockDeep<Callback>())
  ).rejects.toThrow('auth_time claim does not match current time');
});
